    ...
    export default{
        ...
        if(this.$store.state.memoryUsages.length===0)
        {
            ...
            this.chart1 = 
            {
                ...
                xAxisData: xData,
                seriesData: [
                { name: 'memoryUsage(%)', type: 'line', data: yData },
                ],
                xAxisName: '时间轴', 
        ...
        updatechart1(){
            let tmpmemoryinfo = 
                {
                    ...
                    seriesData: [
                        { name: "memoryUsage(%)", type: 'line', data: this.$store.state.memoryUsages }
                    ],
                    xAxisName: '时间轴', 
                    yAxisName: '%', // 设置 yAxisName 的值
                    yAxis_min: 0,
            ...
            this.chart1 = tmpmemoryinfo;
            this.$refs.chartcomponent1.updateData(this.chart1); 
        },
        updatepage(){
            if(this.$store.state.memoryUsages.length!=0){
            const cpuusage = this.$store.state.memoryUsages;
